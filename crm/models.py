...
class LeadQuoteAttachment(models.Model):
    """A quote prepared outside the system (PDF/image/doc) attached to a lead."""
    lead = models.ForeignKey(Lead, on_delete=models.CASCADE, related_name='quote_attachments')
    title = models.CharField(max_length=200, blank=True, help_text='e.g. "Website redesign quote v2"')
    file = models.FileField(upload_to='lead_quotes/')
    amount = models.DecimalField(
        max_digits=12, decimal_places=2, null=True, blank=True,
        help_text='Quoted amount (optional)'
    )
    notes = models.TextField(blank=True)
    created_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        related_name='crm_lead_quote_attachments'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"Quote attachment on {self.lead}"
